...
def parse_xyz_atom_numbers(path: Path) -> np.ndarray:
    ...
    for index, line in enumerate(atom_lines, start=3):
        ...
        atomic_number = PERIODIC_SYMBOL_TO_NUMBER.get(symbol.upper())
        if atomic_number is None:
            raise ValueError(f"Unsupported element symbol {symbol!r} in {path} line {index}.")
        atom_numbers.append(int(atomic_number))
